...
class DeepSeekChat {
  ...
  copyCode(button, text) {
    ...
    navigator.clipboard.writeText(text).then(() => {
      // Разная анимация в зависимости от типа кнопки
      if (button.classList.contains('message-copy-btn')) {
        ...
        // Для кнопок копирования кода - изменение цветов
        button.classList.add('copied');
  ...
  async sendMessage() {
    ...
    alert('sendMessage called, isMobile: ' + this.isMobile + ', message: ' + document.getElementById('message-input').value.trim());
